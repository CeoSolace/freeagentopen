...
const discordVerifyToken_1 = require("../models/discordVerifyToken");
...
class VerificationService {
    constructor() {
        this.verifyButtonCustomId = 'verifyButton';
    }
    /**
     ...
     */
    async ensureVerificationMessage() {
        const channel = await discordClient_1.client.channels.fetch(config_1.env.DISCORD_VERIFY_CHANNEL_ID);
        if (!channel || !channel.isTextBased()) {
            logger_1.logger.error('Verify channel not found or not text based');
            return;
        }
        try {
            const messages = await channel.messages.fetch({ limit: 50 });
            const existing = messages.find((msg) => {
                if (msg.author.id !== discordClient_1.client.user?.id)
                    return false;
                return msg.components.some((row) => row.components.some((comp) => comp.customId === this.verifyButtonCustomId));
            });
            if (existing) {
                logger_1.logger.info('Verification message already present');
                return;
            }
            ...
            logger_1.logger.info('Sent new verification message');
        }
        catch (err) {
            logger_1.logger.error({ err }, 'Failed to ensure verification message');
        }
    }
    /**
    ...
    async handleVerifyButton(interaction) {
        if (!interaction.isButton())
            return;
        if (interaction.customId !== this.verifyButtonCustomId)
            return;
        ...
        try {
            ...
            await discordVerifyToken_1.DiscordVerifyToken.create({
                discordUserId,
                tokenHash,
                expiresAt,
            });
            const verifyUrl = `${config_1.env.APP_URL}/verify/discord?token=${encodeURIComponent(token)}`;
            ...
            logger_1.logger.info({ discordUserId }, 'Issued verification token');
        }
        catch (err) {
            logger_1.logger.error({ err }, 'Failed to handle verify button');
     ...
     */
    async completeVerification(discordUserId, token) {
        const tokenHash = (0, crypto_1.hashToken)(token);
        const record = await discordVerifyToken_1.DiscordVerifyToken.findOne({ discordUserId, tokenHash }).exec();
        if (!record) {
            return false;
        }
        ...
        await record.deleteOne();
        // Mark user verified
        const user = await user_1.User.findOneAndUpdate({ discordId: discordUserId }, { verified: true }, { upsert: true, new: true }).exec();
        // Assign member role
        await guildService_1.guildService.assignRole(discordUserId, config_1.env.DISCORD_MEMBER_ROLE_ID);
        await user_1.User.updateOne({ discordId: discordUserId }, { memberRoleAssigned: true }).exec();
        logger_1.logger.info({ discordUserId }, 'Verification completed via website');
        return true;
    }
    /**
     ...
     */
    async resendVerification(discordUserId) {
        try {
            ...
            const tokenHash = (0, crypto_1.hashToken)(token);
            const expiresAt = new Date(Date.now() + 15 * 60 * 1000);
            await discordVerifyToken_1.DiscordVerifyToken.create({ discordUserId, tokenHash, expiresAt });
            const verifyUrl = `${config_1.env.APP_URL}/verify/discord?token=${encodeURIComponent(token)}`;
            await user.send(`Here is your new verification link: ${verifyUrl}`);
            logger_1.logger.info({ discordUserId }, 'Resent verification link via DM');
            return true;
        }
        catch (err) {
            logger_1.logger.warn({ err }, 'Failed to resend verification DM');
            return false;
        }
    }
    /**
    ...
    async cleanupExpiredTokens() {
        try {
            const result = await discordVerifyToken_1.DiscordVerifyToken.deleteMany({ expiresAt: { $lt: new Date() } }).exec();
            if (result.deletedCount && result.deletedCount > 0) {
                logger_1.logger.info({ deleted: result.deletedCount }, 'Cleaned up expired verification tokens');
            }
        }
        catch (err) {
            logger_1.logger.error({ err }, 'Failed to clean up verification tokens');
        }
    }
}
